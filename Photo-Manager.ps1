...
function Read-Ini {
    ...
    try {
        foreach ($line in [System.IO.File]::ReadAllLines($script:iniPath)) {
            $line=$line.Trim(); if($line.StartsWith('#') -or $line.StartsWith(';') -or -not $line.Contains('=')){continue}
            $parts=$line.Split('=',2); $key=$parts[0].Trim(); $val=$parts[1].Trim()
            if ($d.ContainsKey($key)) {
                if ($key -match 'Folder|ZoomMode|WindowState|ViewMode|SortColumn|SortDirection'){$d[$key]=$val}
                else{$p=0; if([int]::TryParse($val,[ref]$p)){$d[$key]=$p}}
            }
        }
    } catch {}
    return $d
...
function Write-Ini([hashtable]$cfg) {
    try {
        ...
        $lines=@("# PSPhotoViewer Configuration","")
...
function Save-CurrentConfig {
    ...
    $script:config.LeftPanelVisible=if($script:panelVisible){1}else{0}
    ...
    if(-not [string]::IsNullOrEmpty($script:currentFolder)){$script:config.LastFolder=$script:currentFolder}
    Write-Ini $script:config
}
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
  ...
  <Window.Resources>
    ...
    <Style x:Key="DarkVerticalScrollBar" TargetType="ScrollBar"><Setter Property="Width" Value="8"/><Setter Property="MinWidth" Value="8"/><Setter Property="Background" Value="#FF1A1E28"/><Setter Property="Template"><Setter.Value><ControlTemplate TargetType="ScrollBar"><Grid><Border Background="#FF1A1E28" CornerRadius="3"/><Track x:Name="PART_Track" IsDirectionReversed="True"><Track.DecreaseRepeatButton><RepeatButton Style="{StaticResource DarkScrollBarPageButton}" Command="ScrollBar.PageUpCommand"/></Track.DecreaseRepeatButton><Track.Thumb><Thumb Style="{StaticResource DarkScrollBarThumb}"/></Track.Thumb><Track.IncreaseRepeatButton><RepeatButton Style="{StaticResource DarkScrollBarPageButton}" Command="ScrollBar.PageDownCommand"/></Track.IncreaseRepeatButton></Track></Grid></ControlTemplate></Setter.Value></Setter></Style>
    <Style x:Key="DarkHorizontalScrollBar" TargetType="ScrollBar"><Setter Property="Height" Value="8"/><Setter Property="MinHeight" Value="8"/><Setter Property="Background" Value="#FF1A1E28"/><Setter Property="Template"><Setter.Value><ControlTemplate TargetType="ScrollBar"><Grid><Border Background="#FF1A1E28" CornerRadius="3"/><Track x:Name="PART_Track" IsDirectionReversed="False"><Track.DecreaseRepeatButton><RepeatButton Style="{StaticResource DarkScrollBarPageButton}" Command="ScrollBar.PageLeftCommand"/></Track.DecreaseRepeatButton><Track.Thumb><Thumb Style="{StaticResource DarkScrollBarThumb}"/></Track.Thumb><Track.IncreaseRepeatButton><RepeatButton Style="{StaticResource DarkScrollBarPageButton}" Command="ScrollBar.PageRightCommand"/></Track.IncreaseRepeatButton></Track></Grid></ControlTemplate></Setter.Value></Setter></Style>
    <Style x:Key="DarkScrollViewer" TargetType="ScrollViewer"><Setter Property="Template"><Setter.Value><ControlTemplate TargetType="ScrollViewer"><Grid><Grid.ColumnDefinitions><ColumnDefinition Width="*"/><ColumnDefinition Width="Auto"/></Grid.ColumnDefinitions><Grid.RowDefinitions><RowDefinition Height="*"/><RowDefinition Height="Auto"/></Grid.RowDefinitions><ScrollContentPresenter Grid.Column="0" Grid.Row="0"/><ScrollBar x:Name="PART_VerticalScrollBar" Grid.Column="1" Grid.Row="0" Style="{StaticResource DarkVerticalScrollBar}" Orientation="Vertical" Value="{TemplateBinding VerticalOffset}" Maximum="{TemplateBinding ScrollableHeight}" ViewportSize="{TemplateBinding ViewportHeight}" Visibility="{TemplateBinding ComputedVerticalScrollBarVisibility}"/><ScrollBar x:Name="PART_HorizontalScrollBar" Grid.Column="0" Grid.Row="1" Style="{StaticResource DarkHorizontalScrollBar}" Orientation="Horizontal" Value="{TemplateBinding HorizontalOffset}" Maximum="{TemplateBinding ScrollableWidth}" ViewportSize="{TemplateBinding ViewportWidth}" Visibility="{TemplateBinding ComputedHorizontalScrollBarVisibility}"/></Grid></ControlTemplate></Setter.Value></Setter></Style>
    <Style x:Key="NavPillBtn" TargetType="Button"><Setter Property="Cursor" Value="Hand"/><Setter Property="Height" Value="34"/><Setter Property="Width" Value="48"/><Setter Property="Template"><Setter.Value><ControlTemplate TargetType="Button"><Grid><Border x:Name="bd" CornerRadius="5" BorderThickness="1" BorderBrush="#FF4A6A90" Background="{StaticResource PillNormal}"/><ContentPresenter HorizontalAlignment="Center" VerticalAlignment="Center"/></Grid><ControlTemplate.Triggers><Trigger Property="IsMouseOver" Value="True"><Setter TargetName="bd" Property="Background" Value="{StaticResource PillHover}"/></Trigger><Trigger Property="IsPressed" Value="True"><Setter TargetName="bd" Property="Background" Value="{StaticResource PillPressed}"/></Trigger><Trigger Property="IsEnabled" Value="False"><Setter TargetName="bd" Property="Opacity" Value="0.35"/></Trigger></ControlTemplate.Triggers></ControlTemplate></Setter.Value></Setter></Style>
    <Style x:Key="OrbBtn" TargetType="Button"><Setter Property="Cursor" Value="Hand"/><Setter Property="Width" Value="52"/><Setter Property="Height" Value="52"/><Setter Property="Template"><Setter.Value><ControlTemplate TargetType="Button"><Grid Width="52" Height="52"><Ellipse Margin="-6"><Ellipse.Fill><SolidColorBrush Color="#5000AAFF"/></Ellipse.Fill><Ellipse.Effect><BlurEffect Radius="10"/></Ellipse.Effect></Ellipse><Ellipse Fill="#AA000000" Margin="2,4,2,-2"><Ellipse.Effect><BlurEffect Radius="5"/></Ellipse.Effect></Ellipse><Ellipse><Ellipse.Fill><LinearGradientBrush StartPoint="0,0" EndPoint="0,1"><GradientStop Color="#FF8899AA" Offset="0"/><GradientStop Color="#FF3A4A5A" Offset="1"/></LinearGradientBrush></Ellipse.Fill></Ellipse><Ellipse Margin="3" Fill="#FF05101A"/><Ellipse x:Name="orbBody" Margin="4"><Ellipse.Fill><RadialGradientBrush GradientOrigin="0.5,1.05" Center="0.5,1.05" RadiusX="0.85" RadiusY="0.9"><GradientStop Color="#FF60B8FF" Offset="0"/><GradientStop Color="#FF1870D0" Offset="0.5"/><GradientStop Color="#FF083880" Offset="1"/></RadialGradientBrush></Ellipse.Fill></Ellipse><Path Margin="5,4,5,0" Stretch="Fill" Height="24" VerticalAlignment="Top" Data="M 0,16 C 8,1 44,1 52,16 C 44,25 8,25 0,16 Z"><Path.Fill><LinearGradientBrush StartPoint="0.5,0" EndPoint="0.5,1"><GradientStop Color="#DDFFFFFF" Offset="0"/><GradientStop Color="#44FFFFFF" Offset="0.55"/><GradientStop Color="#00FFFFFF" Offset="1"/></LinearGradientBrush></Path.Fill></Path><Canvas Width="52" Height="52"><Path Fill="#EEFFFFFF" Canvas.Left="20" Canvas.Top="17" Data="M 0,0 L 16,9 L 0,18 Z"/></Canvas></Grid><ControlTemplate.Triggers><Trigger Property="IsMouseOver" Value="True"><Setter TargetName="orbBody" Property="Opacity" Value="0.88"/></Trigger><Trigger Property="IsPressed" Value="True"><Setter TargetName="orbBody" Property="Opacity" Value="0.7"/></Trigger></ControlTemplate.Triggers></ControlTemplate></Setter.Value></Setter></Style>
    <Style x:Key="ToolBtn" TargetType="Button"><Setter Property="Cursor" Value="Hand"/><Setter Property="Width" Value="34"/><Setter Property="Height" Value="34"/><Setter Property="Template"><Setter.Value><ControlTemplate TargetType="Button"><Border x:Name="bd" CornerRadius="4" BorderThickness="1" BorderBrush="Transparent" Background="Transparent" Padding="5"><ContentPresenter HorizontalAlignment="Center" VerticalAlignment="Center"/></Border><ControlTemplate.Triggers><Trigger Property="IsMouseOver" Value="True"><Setter TargetName="bd" Property="BorderBrush" Value="#FF4A6A90"/><Setter TargetName="bd" Property="Background" Value="{StaticResource PillHover}"/></Trigger><Trigger Property="IsPressed" Value="True"><Setter TargetName="bd" Property="Background" Value="{StaticResource PillPressed}"/></Trigger><Trigger Property="IsEnabled" Value="False"><Setter TargetName="bd" Property="Opacity" Value="0.3"/></Trigger></ControlTemplate.Triggers></ControlTemplate></Setter.Value></Setter></Style>
    <Style x:Key="OverlayBtn" TargetType="Button"><Setter Property="Foreground" Value="White"/><Setter Property="Cursor" Value="Hand"/><Setter Property="Template"><Setter.Value><ControlTemplate TargetType="Button"><Border x:Name="bd" Background="#55000000" BorderBrush="#55FFFFFF" BorderThickness="1" CornerRadius="4" Padding="12,6"><ContentPresenter HorizontalAlignment="Center" VerticalAlignment="Center"/></Border><ControlTemplate.Triggers><Trigger Property="IsMouseOver" Value="True"><Setter TargetName="bd" Property="Background" Value="#88000000"/></Trigger></ControlTemplate.Triggers></ControlTemplate></Setter.Value></Setter></Style>
    <Style x:Key="SmallToolBtn" TargetType="Button"><Setter Property="Cursor" Value="Hand"/><Setter Property="Width" Value="22"/><Setter Property="Height" Value="22"/><Setter Property="Template"><Setter.Value><ControlTemplate TargetType="Button"><Border x:Name="bd" CornerRadius="3" BorderThickness="1" BorderBrush="#FF3A5070" Background="#FF1A2636" Padding="2"><ContentPresenter HorizontalAlignment="Center" VerticalAlignment="Center"/></Border><ControlTemplate.Triggers><Trigger Property="IsMouseOver" Value="True"><Setter TargetName="bd" Property="BorderBrush" Value="#FF6A9AC0"/><Setter TargetName="bd" Property="Background" Value="#FF2A3E58"/></Trigger><Trigger Property="IsPressed" Value="True"><Setter TargetName="bd" Property="Background" Value="#FF1A3050"/></Trigger></ControlTemplate.Triggers></ControlTemplate></Setter.Value></Setter></Style>
    <Style x:Key="ViewModeBtn" TargetType="Button"><Setter Property="Cursor" Value="Hand"/><Setter Property="Width" Value="24"/><Setter Property="Height" Value="22"/><Setter Property="Template"><Setter.Value><ControlTemplate TargetType="Button"><Border x:Name="bd" CornerRadius="3" BorderThickness="1" BorderBrush="#FF3A5070" Background="#FF1A2636" Padding="2"><ContentPresenter HorizontalAlignment="Center" VerticalAlignment="Center"/></Border><ControlTemplate.Triggers><Trigger Property="IsMouseOver" Value="True"><Setter TargetName="bd" Property="BorderBrush" Value="#FF6A9AC0"/><Setter TargetName="bd" Property="Background" Value="#FF2A3E58"/></Trigger><Trigger Property="IsPressed" Value="True"><Setter TargetName="bd" Property="Background" Value="#FF1A3050"/></Trigger></ControlTemplate.Triggers></ControlTemplate></Setter.Value></Setter></Style>
    <Style x:Key="DetailHeaderBtn" TargetType="Button"><Setter Property="Cursor" Value="Hand"/><Setter Property="Height" Value="22"/><Setter Property="HorizontalContentAlignment" Value="Left"/><Setter Property="Template"><Setter.Value><ControlTemplate TargetType="Button"><Border x:Name="bd" Background="#FF151C28" BorderBrush="#FF2A3A4A" BorderThickness="0,0,1,1" Padding="6,2"><ContentPresenter HorizontalAlignment="{TemplateBinding HorizontalContentAlignment}" VerticalAlignment="Center"/></Border><ControlTemplate.Triggers><Trigger Property="IsMouseOver" Value="True"><Setter TargetName="bd" Property="Background" Value="#FF1E2E44"/></Trigger><Trigger Property="IsPressed" Value="True"><Setter TargetName="bd" Property="Background" Value="#FF1A3050"/></Trigger></ControlTemplate.Triggers></ControlTemplate></Setter.Value></Setter></Style>
    <Style TargetType="TreeViewItem"><Setter Property="Foreground" Value="#FFAABBCC"/><Setter Property="FontSize" Value="12"/><Setter Property="Padding" Value="2,1"/><Setter Property="BorderThickness" Value="0"/><Setter Property="Background" Value="Transparent"/><Setter Property="Template"><Setter.Value><ControlTemplate TargetType="TreeViewItem"><Grid><Grid.RowDefinitions><RowDefinition Height="Auto"/><RowDefinition/></Grid.RowDefinitions><Border x:Name="Bd" Grid.Row="0" Background="{TemplateBinding Background}" Padding="{TemplateBinding Padding}" CornerRadius="3" Margin="1"><DockPanel><ToggleButton x:Name="Expander" DockPanel.Dock="Left" Width="16" Height="16" IsChecked="{Binding IsExpanded, RelativeSource={RelativeSource TemplatedParent}}" ClickMode="Press" Background="Transparent" BorderThickness="0" Cursor="Hand"><ToggleButton.Template><ControlTemplate TargetType="ToggleButton"><Border Background="Transparent" Width="16" Height="16"><Path x:Name="arrow" Fill="#FF6A8AA8" HorizontalAlignment="Center" VerticalAlignment="Center" Data="M 0,0 L 6,4 L 0,8 Z"/></Border><ControlTemplate.Triggers><Trigger Property="IsChecked" Value="True"><Setter TargetName="arrow" Property="Data" Value="M 0,0 L 8,0 L 4,6 Z"/></Trigger><Trigger Property="IsMouseOver" Value="True"><Setter TargetName="arrow" Property="Fill" Value="#FFAACCEE"/></Trigger></ControlTemplate.Triggers></ControlTemplate></ToggleButton.Template></ToggleButton><ContentPresenter x:Name="PART_Header" ContentSource="Header" Margin="2,0,0,0"/></DockPanel></Border><ItemsPresenter x:Name="ItemsHost" Grid.Row="1" Margin="18,0,0,0" Visibility="Collapsed"/></Grid><ControlTemplate.Triggers><Trigger Property="IsExpanded" Value="True"><Setter TargetName="ItemsHost" Property="Visibility" Value="Visible"/></Trigger><Trigger Property="HasItems" Value="False"><Setter TargetName="Expander" Property="Visibility" Value="Hidden"/></Trigger><Trigger Property="IsSelected" Value="True"><Setter TargetName="Bd" Property="Background" Value="#FF1A3A5A"/></Trigger><MultiTrigger><MultiTrigger.Conditions><Condition Property="IsSelected" Value="False"/><Condition Property="IsMouseOver" Value="True"/></MultiTrigger.Conditions><Setter TargetName="Bd" Property="Background" Value="#FF1E2E44"/></MultiTrigger></ControlTemplate.Triggers></ControlTemplate></Setter.Value></Setter></Style>
  </Window.Resources>
  <Grid>
    <DockPanel x:Name="MainView">
      <Grid DockPanel.Dock="Bottom" Height="65">
        ...
        <StackPanel HorizontalAlignment="Right" VerticalAlignment="Bottom" Margin="0,0,10,3" Orientation="Vertical"><TextBlock x:Name="FileNameLabel" Text="" Foreground="#FF8AACCC" FontSize="10" TextAlignment="Right" TextTrimming="CharacterEllipsis" MaxWidth="400"/><TextBlock x:Name="FileInfoLabel" Text="" Foreground="#FF7A8EA8" FontSize="10" TextAlignment="Right"/></StackPanel>
        <Grid VerticalAlignment="Center"><Grid.ColumnDefinitions><ColumnDefinition Width="*"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="*"/></Grid.ColumnDefinitions>
          <StackPanel Grid.Column="0" Orientation="Horizontal" HorizontalAlignment="Right" Margin="0,0,28,0"><Button x:Name="ZoomOutBtn" Style="{StaticResource ToolBtn}" ToolTip="Zoom out"><Canvas Width="20" Height="20"><Ellipse Width="13" Height="13" Stroke="#FFAACCEE" StrokeThickness="1.8"/><Line X1="3.5" Y1="6.5" X2="9.5" Y2="6.5" Stroke="#FFAACCEE" StrokeThickness="1.8" StrokeStartLineCap="Round" StrokeEndLineCap="Round"/><Line X1="10" Y1="10" X2="18" Y2="18" Stroke="#FFAACCEE" StrokeThickness="2" StrokeStartLineCap="Round" StrokeEndLineCap="Round"/></Canvas></Button><Button x:Name="FitBtn" Style="{StaticResource ToolBtn}" ToolTip="Best fit (F)" Margin="2,0"><Canvas Width="20" Height="20"><Path Fill="#FFAACCEE" Data="M 0,0 L 5,0 L 0,5 Z"/><Path Fill="#FFAACCEE" Data="M 15,0 L 20,0 L 20,5 Z"/><Path Fill="#FFAACCEE" Data="M 0,15 L 0,20 L 5,20 Z"/><Path Fill="#FFAACCEE" Data="M 20,15 L 20,20 L 15,20 Z"/></Canvas></Button><Button x:Name="ZoomInBtn" Style="{StaticResource ToolBtn}" ToolTip="Zoom in"><Canvas Width="20" Height="20"><Ellipse Width="13" Height="13" Stroke="#FFAACCEE" StrokeThickness="1.8"/><Line X1="3.5" Y1="6.5" X2="9.5" Y2="6.5" Stroke="#FFAACCEE" StrokeThickness="1.8" StrokeStartLineCap="Round" StrokeEndLineCap="Round"/><Line X1="6.5" Y1="3.5" X2="6.5" Y2="9.5" Stroke="#FFAACCEE" StrokeThickness="1.8" StrokeStartLineCap="Round" StrokeEndLineCap="Round"/><Line X1="10" Y1="10" X2="18" Y2="18" Stroke="#FFAACCEE" StrokeThickness="2" StrokeStartLineCap="Round" StrokeEndLineCap="Round"/></Canvas></Button></StackPanel>
          <StackPanel Grid.Column="1" Orientation="Horizontal" HorizontalAlignment="Center"><Button x:Name="PrevBtn" Style="{StaticResource NavPillBtn}" ToolTip="Previous" Margin="0,0,6,0"><Canvas Width="18" Height="16"><Rectangle Width="2.8" Height="14" Fill="#FFCCDDED" Canvas.Left="1" Canvas.Top="1" RadiusX="1" RadiusY="1"/><Path Fill="#FFCCDDED" Data="M 17,1 L 6,8 L 17,15 Z"/></Canvas></Button><Button x:Name="FSBtn" Style="{StaticResource OrbBtn}" ToolTip="Slideshow (F11)" Margin="0,0,6,0"/><Button x:Name="NextBtn" Style="{StaticResource NavPillBtn}" ToolTip="Next"><Canvas Width="18" Height="16"><Path Fill="#FFCCDDED" Data="M 1,1 L 12,8 L 1,15 Z"/><Rectangle Width="2.8" Height="14" Fill="#FFCCDDED" Canvas.Left="14.2" Canvas.Top="1" RadiusX="1" RadiusY="1"/></Canvas></Button></StackPanel>
      ...
      <Grid><Grid.ColumnDefinitions><ColumnDefinition Width="Auto"/><ColumnDefinition x:Name="LeftPanelColumn" Width="280" MinWidth="140"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="*" MinWidth="100"/></Grid.ColumnDefinitions>
        ...
        <DockPanel x:Name="LeftPanel" Grid.Column="1" Background="#FF10151C">
          ...
          <Border DockPanel.Dock="Bottom" Padding="6,3,6,3"><DockPanel><StackPanel DockPanel.Dock="Right" Orientation="Horizontal" Margin="4,0,0,0"><StackPanel x:Name="ThumbSizePanel" Orientation="Horizontal"><Button x:Name="ThumbSmallerBtn" Style="{StaticResource SmallToolBtn}" ToolTip="Smaller" Margin="0,0,2,0"><TextBlock Text="-" Foreground="#FFAACCEE" FontSize="12" FontWeight="Bold" HorizontalAlignment="Center" VerticalAlignment="Center" Margin="0,-3,0,0"/></Button><TextBlock x:Name="ThumbSizeLabel" Text="64" Foreground="#FF5A7A98" FontSize="9" VerticalAlignment="Center" Margin="2,0,2,0" MinWidth="18" TextAlignment="Center"/><Button x:Name="ThumbLargerBtn" Style="{StaticResource SmallToolBtn}" ToolTip="Larger" Margin="0,0,4,0"><TextBlock Text="+" Foreground="#FFAACCEE" FontSize="12" FontWeight="Bold" HorizontalAlignment="Center" VerticalAlignment="Center" Margin="0,-3,0,0"/></Button></StackPanel><Border Width="1" Background="#FF2A3A4A" Margin="2,2,4,2"/><Button x:Name="ViewThumbBtn" Style="{StaticResource ViewModeBtn}" ToolTip="Thumbnails" Margin="0,0,2,0"><Canvas Width="14" Height="14"><Rectangle Width="6" Height="6" Fill="#FFAACCEE"/><Rectangle Width="6" Height="6" Fill="#FFAACCEE" Canvas.Left="8"/><Rectangle Width="6" Height="6" Fill="#FFAACCEE" Canvas.Top="8"/><Rectangle Width="6" Height="6" Fill="#FFAACCEE" Canvas.Left="8" Canvas.Top="8"/></Canvas></Button><Button x:Name="ViewDetailBtn" Style="{StaticResource ViewModeBtn}" ToolTip="Details"><Canvas Width="14" Height="14"><Rectangle Width="3" Height="3" Fill="#FF6A8AA8" Canvas.Top="0.5" RadiusX="0.5" RadiusY="0.5"/><Rectangle Width="9" Height="2.5" Fill="#FFAACCEE" Canvas.Left="5" Canvas.Top="0.5"/><Rectangle Width="3" Height="3" Fill="#FF6A8AA8" Canvas.Top="5.5" RadiusX="0.5" RadiusY="0.5"/><Rectangle Width="9" Height="2.5" Fill="#FFAACCEE" Canvas.Left="5" Canvas.Top="5.5"/><Rectangle Width="3" Height="3" Fill="#FF6A8AA8" Canvas.Top="10.5" RadiusX="0.5" RadiusY="0.5"/><Rectangle Width="9" Height="2.5" Fill="#FFAACCEE" Canvas.Left="5" Canvas.Top="10.5"/></Canvas></Button></StackPanel><TextBlock x:Name="PreviewHeader" Text="IMAGES (0)" Foreground="#FF6A8AA8" FontSize="10" FontWeight="SemiBold" VerticalAlignment="Center"/></DockPanel></Border>
          <Border DockPanel.Dock="Bottom" Margin="4,2,4,4" CornerRadius="4" Background="#FF0C1018" BorderBrush="#FF2A3A4A" BorderThickness="1" Height="300" x:Name="PreviewListBorder"><DockPanel><Grid x:Name="DetailHeaderRow" DockPanel.Dock="Top" Height="22" Visibility="Collapsed"><Grid.ColumnDefinitions><ColumnDefinition x:Name="DetailColNameDef" Width="200" MinWidth="60"/><ColumnDefinition Width="Auto"/><ColumnDefinition x:Name="DetailColSizeDef" Width="80" MinWidth="40"/><ColumnDefinition Width="Auto"/><ColumnDefinition x:Name="DetailColDateDef" Width="140" MinWidth="60"/></Grid.ColumnDefinitions><Button x:Name="SortNameBtn" Grid.Column="0" Style="{StaticResource DetailHeaderBtn}"><TextBlock x:Name="SortNameText" Text="Name" Foreground="#FF8AACCC" FontSize="10"/></Button><GridSplitter Grid.Column="1" Width="3" Background="#FF2A3A4A" ResizeBehavior="PreviousAndNext" Cursor="SizeWE"/><Button x:Name="SortSizeBtn" Grid.Column="2" Style="{StaticResource DetailHeaderBtn}" HorizontalContentAlignment="Right"><TextBlock x:Name="SortSizeText" Text="Size" Foreground="#FF8AACCC" FontSize="10"/></Button><GridSplitter Grid.Column="3" Width="3" Background="#FF2A3A4A" ResizeBehavior="PreviousAndNext" Cursor="SizeWE"/><Button x:Name="SortDateBtn" Grid.Column="4" Style="{StaticResource DetailHeaderBtn}"><TextBlock x:Name="SortDateText" Text="Date modified" Foreground="#FF8AACCC" FontSize="10"/></Button></Grid><ListBox x:Name="ImagePreviewList" Background="Transparent" BorderThickness="0" ScrollViewer.HorizontalScrollBarVisibility="Disabled" ScrollViewer.VerticalScrollBarVisibility="Auto" VirtualizingStackPanel.IsVirtualizing="True" VirtualizingStackPanel.VirtualizationMode="Recycling"><ListBox.Resources><Style TargetType="ScrollBar" BasedOn="{StaticResource DarkVerticalScrollBar}"/></ListBox.Resources><ListBox.ItemContainerStyle><Style TargetType="ListBoxItem"><Setter Property="Padding" Value="4,2"/><Setter Property="Margin" Value="1"/><Setter Property="Cursor" Value="Hand"/><Setter Property="HorizontalContentAlignment" Value="Stretch"/><Setter Property="Template"><Setter.Value><ControlTemplate TargetType="ListBoxItem"><Border x:Name="bd" CornerRadius="3" Padding="4,2" Background="Transparent" BorderThickness="1" BorderBrush="Transparent"><ContentPresenter/></Border><ControlTemplate.Triggers><Trigger Property="IsMouseOver" Value="True"><Setter TargetName="bd" Property="Background" Value="#FF1E2E44"/><Setter TargetName="bd" Property="BorderBrush" Value="#FF3A5A78"/></Trigger><Trigger Property="IsSelected" Value="True"><Setter TargetName="bd" Property="Background" Value="#FF1A3A5A"/><Setter TargetName="bd" Property="BorderBrush" Value="#FF4A7AAA"/></Trigger></ControlTemplate.Triggers></ControlTemplate></Setter.Value></Setter></Style></ListBox.ItemContainerStyle></ListBox></DockPanel></Border>
        ...
        <Grid x:Name="ImageContainer" Grid.Column="3" Background="#FF0C1018" ClipToBounds="True">
          <ScrollViewer x:Name="Viewer" Style="{StaticResource DarkScrollViewer}" HorizontalScrollBarVisibility="Auto" VerticalScrollBarVisibility="Auto" Background="Transparent"><Grid x:Name="ImageWrapper" Background="Transparent"><Image x:Name="MainImage" Stretch="None" HorizontalAlignment="Center" VerticalAlignment="Center" RenderOptions.BitmapScalingMode="HighQuality"><Image.LayoutTransform><ScaleTransform x:Name="ImageScale" ScaleX="1" ScaleY="1"/></Image.LayoutTransform></Image></Grid></ScrollViewer>
          ...
          <StackPanel x:Name="PlaceholderPanel" HorizontalAlignment="Center" VerticalAlignment="Center" Visibility="Visible"><Border Width="96" Height="76" CornerRadius="6" Margin="0,0,0,16" BorderBrush="#FF3A5070" BorderThickness="1"><Border.Background><LinearGradientBrush StartPoint="0,0" EndPoint="0,1"><GradientStop Color="#FF1E2E44" Offset="0"/><GradientStop Color="#FF111820" Offset="1"/></LinearGradientBrush></Border.Background><Canvas Width="56" Height="44" HorizontalAlignment="Center" VerticalAlignment="Center"><Rectangle Width="56" Height="44" Stroke="#FF3A6090" StrokeThickness="1.5" RadiusX="3" RadiusY="3" Fill="#FF0C1820"/><Ellipse Width="10" Height="10" Fill="#FF4488AA" Canvas.Left="6" Canvas.Top="5"/><Path Fill="#FF2A5A40" Data="M 0,40 L 18,24 L 32,34 L 44,18 L 56,40 Z"/></Canvas></Border><TextBlock Text="No image selected" Foreground="#FF8AA8CC" FontSize="18" FontWeight="Light" TextAlignment="Center" Margin="0,0,0,6"/><TextBlock Text="Press Ctrl+O or use the left panel" Foreground="#FF4A6080" FontSize="11" TextAlignment="Center" Margin="0,0,0,20"/><Button x:Name="PlaceholderOpenBtn" Style="{StaticResource OverlayBtn}" Content="Open Image..." HorizontalAlignment="Center" Foreground="#FFCCDDED" FontSize="12"/></StackPanel>
    ...
    <Border x:Name="FullScreenOverlay" Visibility="Collapsed" Background="Black" Focusable="True" Panel.ZIndex="200"><Grid><Image x:Name="FSImage" Stretch="Uniform" HorizontalAlignment="Center" VerticalAlignment="Center" RenderOptions.BitmapScalingMode="HighQuality"/><Border x:Name="FSBottomBar" VerticalAlignment="Bottom" HorizontalAlignment="Center" Margin="0,0,0,24" Background="#BB000000" CornerRadius="6" Padding="16,7"><StackPanel Orientation="Horizontal"><Button x:Name="FSPrevBtn" Background="Transparent" BorderThickness="0" Cursor="Hand" Width="38" Height="34" Margin="0,0,6,0"><Path Fill="White" Data="M 16,4 L 4,18 L 16,32 Z" Stretch="Uniform" Margin="10,6"/></Button><Button x:Name="FSPauseBtn" Background="Transparent" BorderThickness="0" Cursor="Hand" Width="38" Height="34" Margin="0,0,6,0"><Canvas Width="20" Height="20"><Rectangle Width="5" Height="16" Fill="White" Canvas.Left="2" Canvas.Top="2"/><Rectangle Width="5" Height="16" Fill="White" Canvas.Left="13" Canvas.Top="2"/></Canvas></Button><Button x:Name="FSExitBtn" Background="Transparent" BorderThickness="0" Cursor="Hand" Width="38" Height="34" Margin="0,0,6,0"><Path Stroke="White" StrokeThickness="2.5" StrokeStartLineCap="Round" StrokeEndLineCap="Round" Data="M 4,4 L 16,16 M 16,4 L 4,16" Stretch="Uniform" Margin="10,10"/></Button><Button x:Name="FSNextBtn" Background="Transparent" BorderThickness="0" Cursor="Hand" Width="38" Height="34"><Path Fill="White" Data="M 4,4 L 16,18 L 4,32 Z" Stretch="Uniform" Margin="10,6"/></Button></StackPanel></Border><Border x:Name="FSCountLabelBorder" Background="#88000000" CornerRadius="3" HorizontalAlignment="Right" VerticalAlignment="Top" Margin="0,12,18,0" Padding="4,2"><TextBlock x:Name="FSCountLabel" Text="" Foreground="White" FontSize="13"/></Border></Grid></Border>
  </Grid>
</Window>
"@
...
$PlaceholderPanel=$window.FindName("PlaceholderPanel");$PlaceholderOpenBtn=$window.FindName("PlaceholderOpenBtn");$FileInfoLabel=$window.FindName("FileInfoLabel");$FileCountLabel=$window.FindName("FileCountLabel");$FileNameLabel=$window.FindName("FileNameLabel")
...
$ZoomInBtn=$window.FindName("ZoomInBtn");$ZoomOutBtn=$window.FindName("ZoomOutBtn");$FitBtn=$window.FindName("FitBtn");$FSBtn=$window.FindName("FSBtn");$NextBtn=$window.FindName("NextBtn");$PrevBtn=$window.FindName("PrevBtn")
$FSNextBtn=$window.FindName("FSNextBtn");$FSPrevBtn=$window.FindName("FSPrevBtn");$FSPauseBtn=$window.FindName("FSPauseBtn");$FSExitBtn=$window.FindName("FSExitBtn");$DeleteBtn=$window.FindName("DeleteBtn")
$MenuOpen=$window.FindName("MenuOpen");$MenuFit=$window.FindName("MenuFit");$MenuActual=$window.FindName("MenuActual")
...
$ViewThumbBtn=$window.FindName("ViewThumbBtn");$ViewDetailBtn=$window.FindName("ViewDetailBtn");$HamburgerBtn=$window.FindName("HamburgerBtn");$LeftPanel=$window.FindName("LeftPanel");$MainSplitter=$window.FindName("MainSplitter")
$DetailHeaderRow=$window.FindName("DetailHeaderRow");$SortNameBtn=$window.FindName("SortNameBtn");$SortSizeBtn=$window.FindName("SortSizeBtn");$SortDateBtn=$window.FindName("SortDateBtn")
...
function Create-Thumbnail([string]$path,[int]$size=64){
    try{$stream=New-Object System.IO.FileStream($path,[System.IO.FileMode]::Open,[System.IO.FileAccess]::Read,[System.IO.FileShare]::ReadWrite,4096,[System.IO.FileOptions]::SequentialScan)
    try{$bmp=New-Object System.Windows.Media.Imaging.BitmapImage;$bmp.BeginInit();$bmp.CacheOption=[System.Windows.Media.Imaging.BitmapCacheOption]::OnLoad;$bmp.StreamSource=$stream;$bmp.DecodePixelWidth=$size;$bmp.EndInit();$bmp.Freeze();return $bmp}finally{$stream.Dispose()}}catch{return $null}
}

function New-PictureIcon{$c=New-Object System.Windows.Controls.Canvas;$c.Width=12;$c.Height=12;$c.Margin=[System.Windows.Thickness]::new(0,0,5,0);$f=New-Object System.Windows.Shapes.Rectangle;$f.Width=12;$f.Height=10;$f.RadiusX=1;$f.RadiusY=1;$f.Stroke=[System.Windows.Media.SolidColorBrush]::new([System.Windows.Media.ColorConverter]::ConvertFromString("#FF6A8AA8"));$f.StrokeThickness=1.2;$f.Fill=[System.Windows.Media.SolidColorBrush]::new([System.Windows.Media.ColorConverter]::ConvertFromString("#FF182838"));[System.Windows.Controls.Canvas]::SetTop($f,1);$c.Children.Add($f)|Out-Null;$su=New-Object System.Windows.Shapes.Ellipse;$su.Width=3;$su.Height=3;$su.Fill=[System.Windows.Media.SolidColorBrush]::new([System.Windows.Media.ColorConverter]::ConvertFromString("#FF88AACC"));[System.Windows.Controls.Canvas]::SetLeft($su,2);[System.Windows.Controls.Canvas]::SetTop($su,3);$c.Children.Add($su)|Out-Null;$m=New-Object System.Windows.Shapes.Path;$m.Fill=[System.Windows.Media.SolidColorBrush]::new([System.Windows.Media.ColorConverter]::ConvertFromString("#FF3A7A5A"));$m.Data=[System.Windows.Media.Geometry]::Parse("M 1,10 L 4,5 L 7,8 L 9,4 L 11,10 Z");$c.Children.Add($m)|Out-Null;return $c}

function New-PreviewElement-Thumbnails([string]$path,[int]$tp){
    $fi=[System.IO.FileInfo]::new($path);$thumb=Create-Thumbnail $path $tp;$sz=Format-FileSize $fi.Length
    $st=New-Object System.Windows.Controls.StackPanel;$st.Orientation=[System.Windows.Controls.Orientation]::Vertical;$st.HorizontalAlignment=[System.Windows.HorizontalAlignment]::Stretch
    $tb=New-Object System.Windows.Controls.Border;$tb.Width=$tp+4;$tb.Height=$tp+4;$tb.CornerRadius=[System.Windows.CornerRadius]::new(3);$tb.Background=[System.Windows.Media.SolidColorBrush]::new([System.Windows.Media.ColorConverter]::ConvertFromString("#FF0A0E14"));$tb.BorderBrush=[System.Windows.Media.SolidColorBrush]::new([System.Windows.Media.ColorConverter]::ConvertFromString("#FF2A3A4A"));$tb.BorderThickness=[System.Windows.Thickness]::new(1);$tb.HorizontalAlignment=[System.Windows.HorizontalAlignment]::Center
    $img=New-Object System.Windows.Controls.Image;$img.Source=$thumb;$img.Stretch=[System.Windows.Media.Stretch]::Uniform;$img.HorizontalAlignment=[System.Windows.HorizontalAlignment]::Center;$img.VerticalAlignment=[System.Windows.VerticalAlignment]::Center;$img.Margin=[System.Windows.Thickness]::new(2);[System.Windows.Media.RenderOptions]::SetBitmapScalingMode($img,[System.Windows.Media.BitmapScalingMode]::HighQuality);$tb.Child=$img;$st.Children.Add($tb)|Out-Null
    $nb=New-Object System.Windows.Controls.TextBlock;$nb.Text=$fi.Name;$nb.Foreground=[System.Windows.Media.SolidColorBrush]::new([System.Windows.Media.ColorConverter]::ConvertFromString("#FFBBCCDD"));$nb.FontSize=9;$nb.TextTrimming=[System.Windows.TextTrimming]::CharacterEllipsis;$nb.TextAlignment=[System.Windows.TextAlignment]::Center;$nb.HorizontalAlignment=[System.Windows.HorizontalAlignment]::Center;$nb.Margin=[System.Windows.Thickness]::new(2,3,2,0);$nb.ToolTip=$fi.Name;$nb.MaxWidth=$tp+20;$st.Children.Add($nb)|Out-Null
    $sb=New-Object System.Windows.Controls.TextBlock;$sb.Text=$sz;$sb.Foreground=[System.Windows.Media.SolidColorBrush]::new([System.Windows.Media.ColorConverter]::ConvertFromString("#FF5A7A98"));$sb.FontSize=8;$sb.TextAlignment=[System.Windows.TextAlignment]::Center;$sb.HorizontalAlignment=[System.Windows.HorizontalAlignment]::Center;$sb.Margin=[System.Windows.Thickness]::new(0,1,0,2);$st.Children.Add($sb)|Out-Null;return $st
}

function New-PreviewElement-Details([string]$path){
    $fi=[System.IO.FileInfo]::new($path);$szS=Format-FileSize $fi.Length;$dtS=$fi.LastWriteTime.ToString("g")
    $g=New-Object System.Windows.Controls.Grid;$g.Height=22
    ...
    $g.ColumnDefinitions.Add($c0);$g.ColumnDefinitions.Add($c1);$g.ColumnDefinitions.Add($c2);$g.ColumnDefinitions.Add($c3);$g.ColumnDefinitions.Add($c4)
    $np=New-Object System.Windows.Controls.StackPanel;$np.Orientation=[System.Windows.Controls.Orientation]::Horizontal;$np.VerticalAlignment=[System.Windows.VerticalAlignment]::Center;$np.Children.Add((New-PictureIcon))|Out-Null
    $nb=New-Object System.Windows.Controls.TextBlock;$nb.Text=$fi.Name;$nb.Foreground=[System.Windows.Media.SolidColorBrush]::new([System.Windows.Media.ColorConverter]::ConvertFromString("#FFBBCCDD"));$nb.FontSize=11;$nb.TextTrimming=[System.Windows.TextTrimming]::CharacterEllipsis;$nb.ToolTip=$fi.Name;$nb.VerticalAlignment=[System.Windows.VerticalAlignment]::Center;$np.Children.Add($nb)|Out-Null;[System.Windows.Controls.Grid]::SetColumn($np,0);$g.Children.Add($np)|Out-Null
    $sb=New-Object System.Windows.Controls.TextBlock;$sb.Text=$szS;$sb.Foreground=[System.Windows.Media.SolidColorBrush]::new([System.Windows.Media.ColorConverter]::ConvertFromString("#FF7A9AB8"));$sb.FontSize=10;$sb.VerticalAlignment=[System.Windows.VerticalAlignment]::Center;$sb.TextAlignment=[System.Windows.TextAlignment]::Right;$sb.Margin=[System.Windows.Thickness]::new(4,0,4,0);[System.Windows.Controls.Grid]::SetColumn($sb,2);$g.Children.Add($sb)|Out-Null
    $db=New-Object System.Windows.Controls.TextBlock;$db.Text=$dtS;$db.Foreground=[System.Windows.Media.SolidColorBrush]::new([System.Windows.Media.ColorConverter]::ConvertFromString("#FF5A7A98"));$db.FontSize=10;$db.VerticalAlignment=[System.Windows.VerticalAlignment]::Center;$db.Margin=[System.Windows.Thickness]::new(4,0,2,0);[System.Windows.Controls.Grid]::SetColumn($db,4);$g.Children.Add($db)|Out-Null;return $g
}

function New-PreviewElement([string]$path,[int]$tp){if($script:viewMode -eq "Details"){New-PreviewElement-Details $path}else{New-PreviewElement-Thumbnails $path $tp}}

function Get-SortedFiles([string[]]$files){$infos=$files|ForEach-Object{[System.IO.FileInfo]::new($_)};switch($script:sortColumn){"Size"{$sorted=if($script:sortDirection -eq "Ascending"){$infos|Sort-Object Length}else{$infos|Sort-Object Length -Descending}}"Date"{$sorted=if($script:sortDirection -eq "Ascending"){$infos|Sort-Object LastWriteTime}else{$infos|Sort-Object LastWriteTime -Descending}}default{$sorted=if($script:sortDirection -eq "Ascending"){$infos|Sort-Object Name}else{$infos|Sort-Object Name -Descending}}};return @($sorted|ForEach-Object{$_.FullName})}

function Add-LoadMoreItem{$mb=New-Object System.Windows.Controls.TextBlock;$mb.Text="... $($script:images.Count-$script:batchMax) more - click to load all";$mb.Foreground=[System.Windows.Media.SolidColorBrush]::new([System.Windows.Media.ColorConverter]::ConvertFromString("#FF6A9AC0"));$mb.FontSize=11;$mb.TextAlignment=[System.Windows.TextAlignment]::Center;$mb.HorizontalAlignment=[System.Windows.HorizontalAlignment]::Center;$mb.Margin=[System.Windows.Thickness]::new(0,6,0,6);$mb.Cursor=[System.Windows.Input.Cursors]::Hand;$ImagePreviewList.Items.Add($mb)|Out-Null}
...
function Populate-ImagePreviews([string]$folderPath){
    ...
    $script:batchIndex=0;$script:batchSize=10;$script:batchMax=[Math]::Min($script:images.Count,$script:maxPreviewLoad);$script:fileList=$script:images.ToArray()
    $script:batchAction=[System.Action]{$s=$script:batchIndex;$e=[Math]::Min($s+$script:batchSize,$script:batchMax);for($i=$s;$i -lt $e;$i++){$ImagePreviewList.Items.Add((New-PreviewElement $script:fileList[$i] $script:thumbSize))|Out-Null};$script:batchIndex=$e
        ...
        elseif($script:batchMax -lt $script:images.Count){Add-LoadMoreItem};Request-GC}
    $window.Dispatcher.BeginInvoke([System.Windows.Threading.DispatcherPriority]::Background,$script:batchAction)|Out-Null
}

function Load-MorePreviews{if($ImagePreviewList.Items.Count -gt 0){$l=$ImagePreviewList.Items[$ImagePreviewList.Items.Count-1];if($l -is [System.Windows.Controls.TextBlock]){$ImagePreviewList.Items.RemoveAt($ImagePreviewList.Items.Count-1)}};$sf=$script:batchMax;$script:batchMax=$script:images.Count;$script:batchIndex=$sf
    $script:batchAction=[System.Action]{$s=$script:batchIndex;$e=[Math]::Min($s+$script:batchSize,$script:batchMax);for($i=$s;$i -lt $e;$i++){$ImagePreviewList.Items.Add((New-PreviewElement $script:fileList[$i] $script:thumbSize))|Out-Null};$script:batchIndex=$e;if($script:batchIndex -lt $script:batchMax){$window.Dispatcher.BeginInvoke([System.Windows.Threading.DispatcherPriority]::Background,$script:batchAction)|Out-Null}else{Request-GC}}
    $window.Dispatcher.BeginInvoke([System.Windows.Threading.DispatcherPriority]::Background,$script:batchAction)|Out-Null}
...
function Rebuild-Previews{if($null -eq $script:currentFolder -or -not [System.IO.Directory]::Exists($script:currentFolder)){return};$sortedAll=Get-SortedFiles $script:allFolderFiles;$script:images.Clear();foreach($f in $sortedAll){$script:images.Add($f)};$ImagePreviewList.Items.Clear();if($script:images.Count -eq 0){return}
    ...
    $script:batchAction=[System.Action]{$s=$script:batchIndex;$e=[Math]::Min($s+$script:batchSize,$script:batchMax);for($i=$s;$i -lt $e;$i++){$ImagePreviewList.Items.Add((New-PreviewElement $script:fileList[$i] $script:thumbSize))|Out-Null};$script:batchIndex=$e
        if($script:batchIndex -lt $script:batchMax){$window.Dispatcher.BeginInvoke([System.Windows.Threading.DispatcherPriority]::Background,$script:batchAction)|Out-Null}
        else{if($script:batchMax -lt $script:images.Count){Add-LoadMoreItem};$script:updatingSelection=$true;if($ImagePreviewList.Items.Count -gt $script:currentIndex -and $script:currentIndex -ge 0){$ImagePreviewList.SelectedIndex=$script:currentIndex;$ImagePreviewList.ScrollIntoView($ImagePreviewList.SelectedItem)};$script:updatingSelection=$false;Request-GC}}
    $window.Dispatcher.BeginInvoke([System.Windows.Threading.DispatcherPriority]::Background,$script:batchAction)|Out-Null}

function Build-FolderTree([string]$rp){$FolderTree.Items.Clear();$ri=New-Object System.Windows.Controls.TreeViewItem;$rn=[System.IO.Path]::GetFileName($rp);if([string]::IsNullOrEmpty($rn)){$rn=$rp}
    ...
    $cnt=@([System.IO.Directory]::GetFiles($rp)|Where-Object{$script:supportedExts -contains [System.IO.Path]::GetExtension($_).ToLower()}).Count
    $hp.Children.Add($ic)|Out-Null;$hp.Children.Add($lb)|Out-Null
    if($cnt -gt 0){$cl=New-Object System.Windows.Controls.TextBlock;$cl.Text=" ($cnt)";$cl.Foreground=[System.Windows.Media.SolidColorBrush]::new([System.Windows.Media.ColorConverter]::ConvertFromString("#FF5A7A98"));$cl.FontSize=10;$hp.Children.Add($cl)|Out-Null}
    $ri.Header=$hp;$ri.Tag=$rp;$ri.IsExpanded=$true;Add-SubFolders $ri $rp 0 3;$FolderTree.Items.Add($ri)|Out-Null}

function Add-SubFolders($pi,$pp,$d,$mx){if($d -ge $mx){return};try{$dirs=[System.IO.Directory]::GetDirectories($pp)|Sort-Object}catch{return}
    foreach($dir in $dirs){$dn=[System.IO.Path]::GetFileName($dir);try{$at=[System.IO.File]::GetAttributes($dir);if($at -band [System.IO.FileAttributes]::Hidden){continue};if($at -band [System.IO.FileAttributes]::System){continue}}catch{continue}
        ...
        $ic2=New-Object System.Windows.Controls.Canvas;$ic2.Width=14;$ic2.Height=12;$ic2.Margin=[System.Windows.Thickness]::new(0,2,5,0);$fp2=New-Object System.Windows.Shapes.Path;$fp2.Fill=[System.Windows.Media.SolidColorBrush]::new([System.Windows.Media.ColorConverter]::ConvertFromString("#FF998844"));$fp2.Data=[System.Windows.Media.Geometry]::Parse("M 0,2.5 L 1.5,0 L 6,0 L 7.5,2.5 L 14,2.5 L 14,12 L 0,12 Z");$ic2.Children.Add($fp2)|Out-Null
        $lb2=New-Object System.Windows.Controls.TextBlock;$lb2.Text=$dn;$lb2.Foreground=[System.Windows.Media.SolidColorBrush]::new([System.Windows.Media.ColorConverter]::ConvertFromString("#FFAABBCC"));$sp.Children.Add($ic2)|Out-Null;$sp.Children.Add($lb2)|Out-Null
        try{$cnt=@([System.IO.Directory]::GetFiles($dir)|Where-Object{$script:supportedExts -contains [System.IO.Path]::GetExtension($_).ToLower()}).Count;if($cnt -gt 0){$cl=New-Object System.Windows.Controls.TextBlock;$cl.Text=" ($cnt)";$cl.Foreground=[System.Windows.Media.SolidColorBrush]::new([System.Windows.Media.ColorConverter]::ConvertFromString("#FF5A7A98"));$cl.FontSize=10;$sp.Children.Add($cl)|Out-Null}}catch{}
        $si.Header=$sp;$si.Tag=$dir;Add-SubFolders $si $dir ($d+1) $mx;$pi.Items.Add($si)|Out-Null}}

function Preload-Images{if($script:images.Count -le 1){return};$cp=$script:images[$script:currentIndex];$pi2=($script:currentIndex-1+$script:images.Count)%$script:images.Count;$ni=($script:currentIndex+1)%$script:images.Count;$pp=$script:images[$pi2];$np=$script:images[$ni];$kr=@();foreach($k in $script:imageCache.Keys){if($k -ne $cp -and $k -ne $pp -and $k -ne $np){$kr+=$k}};foreach($k in $kr){$script:imageCache.Remove($k)}
    $pa=[System.Action[string]]{param($pt);if($null -ne $script:imageCache -and -not $script:imageCache.ContainsKey($pt)){try{$script:imageCache[$pt]=[System.IO.File]::ReadAllBytes($pt)}catch{}}};foreach($p in @($np,$pp)){if(-not $script:imageCache.ContainsKey($p)){$window.Dispatcher.BeginInvoke([System.Windows.Threading.DispatcherPriority]::SystemIdle,$pa,$p)|Out-Null}}}

function Load-Image([string]$path){
    try{$bytes=$null;if($script:imageCache.ContainsKey($path)){$bytes=$script:imageCache[$path]}else{$bytes=[System.IO.File]::ReadAllBytes($path)}
    $stream=New-Object System.IO.MemoryStream(,$bytes);try{$bmp=New-Object System.Windows.Media.Imaging.BitmapImage;$bmp.BeginInit();$bmp.CacheOption=[System.Windows.Media.Imaging.BitmapCacheOption]::OnLoad;$bmp.StreamSource=$stream;$bmp.EndInit();$bmp.Freeze();return $bmp}finally{$stream.Dispose()}}catch{return $null}
}
...
# Save on close
$window.Add_Closing({Save-CurrentConfig})
...
$ThumbSmallerBtn.Add_Click({$ci=[Array]::IndexOf($script:thumbSizes,$script:thumbSize);if($ci -lt 0){$ci=2};if($ci -gt 0){$script:thumbSize=$script:thumbSizes[$ci-1];$ThumbSizeLabel.Text="$($script:thumbSize)";Rebuild-Previews;Save-CurrentConfig}})
$ThumbLargerBtn.Add_Click({$ci=[Array]::IndexOf($script:thumbSizes,$script:thumbSize);if($ci -lt 0){$ci=2};if($ci -lt ($script:thumbSizes.Count-1)){$script:thumbSize=$script:thumbSizes[$ci+1];$ThumbSizeLabel.Text="$($script:thumbSize)";Rebuild-Previews;Save-CurrentConfig}})
$ViewThumbBtn.Add_Click({$script:viewMode="Thumbnails";Apply-ViewMode;Rebuild-Previews;Save-CurrentConfig})
$ViewDetailBtn.Add_Click({$script:viewMode="Details";Apply-ViewMode;Rebuild-Previews;Save-CurrentConfig})
$SortNameBtn.Add_Click({if($script:sortColumn -eq "Name"){$script:sortDirection=if($script:sortDirection -eq "Ascending"){"Descending"}else{"Ascending"}}else{$script:sortColumn="Name";$script:sortDirection="Ascending"};Update-SortIndicators;Rebuild-Previews;Save-CurrentConfig})
$SortSizeBtn.Add_Click({if($script:sortColumn -eq "Size"){$script:sortDirection=if($script:sortDirection -eq "Ascending"){"Descending"}else{"Ascending"}}else{$script:sortColumn="Size";$script:sortDirection="Ascending"};Update-SortIndicators;Rebuild-Previews;Save-CurrentConfig})
$SortDateBtn.Add_Click({if($script:sortColumn -eq "Date"){$script:sortDirection=if($script:sortDirection -eq "Ascending"){"Descending"}else{"Ascending"}}else{$script:sortColumn="Date";$script:sortDirection="Descending"};Update-SortIndicators;Rebuild-Previews;Save-CurrentConfig})
...
$MenuOpen.Add_Click({Open-Images});$MenuFit.Add_Click({if($null -ne $MainImage.Source){$script:zoomMode="BestFit";Fit-ToWindow;Save-CurrentConfig}});$MenuActual.Add_Click({if($null -ne $MainImage.Source){$script:zoomMode="ActualSize";Apply-Zoom 1.0;Save-CurrentConfig}})
